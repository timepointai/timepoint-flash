...
def upgrade() -> None:
    ...
    op.add_column(
        "timepoints", sa.Column("blob_written_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.add_column(
        "timepoints",
        sa.Column("nsfw_flag", sa.Boolean(), nullable=False, server_default=sa.text("false")),
    )

    # Refresh tracking
    op.add_column(
        "timepoints",
        sa.Column("generation_version", sa.Integer(), nullable=False, server_default=sa.text("1")),
    )
    ...
    op.add_column(
        "timepoints", sa.Column("last_accessed_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.add_column(
        "timepoints",
        sa.Column("render_type", sa.String(20), nullable=False, server_default="image"),
    )
